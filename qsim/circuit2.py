...
class Circuit:
    ...
    def print(self, maxwidth=None, vals=None):
        ...
        for instructions in self.slots:
            s.add(instructions)
        if vals is not None:
            s.add_endstate(vals)
        print(s)
    ...
    def to_string(self, delim="; "):
        info = [f"qubits={self.qbits}", f"clbits={self.cbits}"]
        lines = ["".join([x + delim for x in info])]
        for slot in self.slots:
            lines.append("SLOT")
            for inst in slot:
                string = inst.to_string()
    ...
    @classmethod
    def from_string2(cls, string, delim="; "):
    ...
    def from_string(cls, string, delim="; "):
        ...
        while lines:
            line = lines.pop(0)
            if line == "SLOT" or line.strip() == "":
                self.slots.add_slot(items)
                items = list()
            else:
                items.append(Instruction.from_string(line, delim))
        if items:
            self.slots.add_slot(items)
        return self
    ...
    def add_measurement(self, qbits, cbits):
        ...
        if not hasattr(cbits, "__len__"):
            cbits = [cbits]
        items = [Measurement("M", q, c) for q, c in zip(qbits, cbits)]
        self.add_instruction(Measurement("M", qbits, cbits))

    def add_gate(self, name, qbits, con=None, n=1, arg=None, argidx=None):
        if hasattr(qbits, "__len__") and len(qbits) != n:
            gates = [Gate(name, q, con=con, arg=arg, argidx=argidx) for q in qbits]
        else:
    ...
    def m(self, qbits=None, cbits=None):
        ...
        if cbits is None:
            cbits = qbits
        # self.add_instruction(Measurement("m", qbits, cbits))
        self.add_measurement(qbits, cbits)
